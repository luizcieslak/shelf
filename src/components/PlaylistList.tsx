...
import { DotsVerticalIcon, CheckCircledIcon, UpdateIcon, ExclamationTriangleIcon } from '@radix-ui/react-icons'
...
const PlaylistList = observer(({ accessToken }: PlaylistListProps) => {
	...
	const transferToYouTube = async (playlist: SpotifyPlaylist) => {
		...
		try {
			...
			const finalStatus = successCount === totalTracks ? 'success' :
				successCount >= totalTracks / 2 ? 'warning' : 'error'
	...
	return (
		<div className="max-w-[600px] mx-auto space-y-4">
			<h2 className='text-2xl font-bold text-gray-900 mb-6'>Your Spotify Playlists</h2>
						...
						<div className="flex items-start justify-between mb-3">
							<div className="flex-1 min-w-0">
								<div className="flex items-center gap-3 mb-2">
									<h3 className="text-base font-semibold text-gray-900 truncate" style={{ fontSize: '16px', fontWeight: 600 }}>
										{playlist.name}
									</h3>
									{/* Spotify Icon */}
									<div className="flex-shrink-0">
										<svg className='w-4 h-4 text-green-600' viewBox='0 0 24 24' fill='currentColor' aria-label="Spotify">
											<title>Spotify</title>
								...
								<p className="text-sm text-gray-600 mb-2">
									{playlist.tracks.total} tracks • Created Recently
								</p>
							</div>
							...
							<div className="mb-4 p-3 bg-gray-50 rounded-md">
								<div className="space-y-2">
									{transferProgress[playlist.id].map((progress, index) => (
										<div key={`${playlist.id}-${progress.step}-${index}`} className="flex items-center gap-2 text-sm">
						...
						<div className={`overflow-hidden transition-all duration-300 ease-in-out ${
							...
							<div className="space-y-3 max-h-80 overflow-y-auto">
								{playlistTracks[playlist.id]?.map(track => (
									<div key={track.id} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-md transition-colors">
